import { MaintenanceTeamEmailed, SirenTestFailed, DripWorkflowTrigger } from '../messages'
...
describe('SirenTestWorkflow', () => {
  ...
  describe('when a siren test is started', () => {
    ...
    beforeAll(async () => bus().publish(new DripWorkflowTrigger(sirenId)))
      ...
      it('should send an email to the maintenance team', async () => {
        const maintenanceEmailReceived = new Promise<void>(resolve => {
          const callback: Listener<{ event: Event }> = ({ event }) => {
            if (event.$name === MaintenanceTeamEmailed.NAME && (event as MaintenanceTeamEmailed).id === sirenId) {
